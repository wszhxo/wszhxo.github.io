...
<servlet>
   <servlet-name>AuthImage</servlet-name>
    <servlet-class>com.zy.servlet.AuthImage</servlet-class>
  </servlet>
  <servlet-mapping>
    <servlet-name>AuthImage</servlet-name>
...
<input type="text" name="checkcode" id="checkcode" size="18"  />
...
``` java
//验证码
    @RequestMapping("/admin/authImage")
...
## 7.layui中的模板里要把img标签的默认id和class去掉

    ```html
     <img src="/admin/authImage"  onclick="javascript:changeImg()" id="img" >
     ```
